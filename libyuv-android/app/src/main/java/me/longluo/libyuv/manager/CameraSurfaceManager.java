package me.longluo.libyuv.manager;
...
import me.longluo.libyuv.DemoApplication;
import me.longluo.libyuv.contacts.Contacts;
import me.longluo.libyuv.listener.CameraPictureListener;
import me.longluo.libyuv.listener.CameraYUVDataListener;
import me.longluo.libyuv.util.CameraUtils;
...
public class CameraSurfaceManager implements SensorEventListener, CameraYUVDataListener {
    ...
    public CameraSurfaceManager(CameraSurfaceView cameraSurfaceView) {
        ...
        mSensorManager = (SensorManager) DemoApplication.getInstance().getSystemService(SENSOR_SERVICE);
    }
    ...
    private boolean isSupport() {
        cropWidth = (int) SPUtils.get(Contacts.CROP_WIDTH, 720);
        cropHeight = (int) SPUtils.get(Contacts.CROP_HEIGHT, 720);
        cropStartX = (int) SPUtils.get(Contacts.CROP_START_X, 0);
        cropStartY = (int) SPUtils.get(Contacts.CROP_START_Y, 0);

        int cameraWidth = mCameraUtil.getCameraWidth();
        int cameraHeight = mCameraUtil.getCameraHeight();
        int scaleWidth = (int) SPUtils.get(Contacts.SCALE_WIDTH, 720);
        int scaleHeight = (int) SPUtils.get(Contacts.SCALE_HEIGHT, 1280);
        ...
        if (cropStartX % 2 != 0 || cropStartY % 2 != 0) {
            Toast.makeText(DemoApplication.getInstance(), "裁剪的开始位置必须为偶数", Toast.LENGTH_SHORT).show();
            return false;
        }
        if (cropStartX + cropWidth > scaleWidth || cropStartY + cropHeight > scaleHeight) {
            Toast.makeText(DemoApplication.getInstance(), "裁剪区域超出范围", Toast.LENGTH_SHORT).show();
            return false;
        }
        return true;
    }
